...
Через конструктор он параметризуется следующими параметрами:
...
- Western Digital Green 1TB 5400rpm HDD

Тестировались файлы размером 1GB, 10GB и 100GB. При этом файлы 1GB и 10GB трестировались на SSD, а файл 100GB - на HDD 5400rpm.

Были получены такие результаты:

|   1GB    |   10GB   |  100GB  |
| -------- | -------- |---------|
| 00:03:12 | 00:40:04 | 7:15:00 |

## Возможные пути улучшения производительности 

- При сортировке по числовому полю происходит сначала загрузка группы записей с одним строковым значением, сортировка их и запись в выходной файл, затем загрузка группы со следующим строковым значением и так далее. Если количество записей в одной такой группе велико, то сортировка будет распараллелена как описано выше в алгоритме. Однако, если количество записей в группе мало и таких групп, идущих одна за другой, много, то все они сортируются по очереди только одним процессором или ядром. Такие маленькие группы, идущие одна за другой, можно было бы сортировать также параллельно, соблюдая порядок записи их в выходной файл. Предполагаемый выигрыш от такой оптимизации мог бы составить около 5%-15%.
